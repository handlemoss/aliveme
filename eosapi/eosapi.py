...
class EosApi:
    ...
    def get_currency_balance(self, account: str, code: str = 'alien.worlds', symbol: str = 'TLM', precision: int = 4):
        try:
            ...
            resp = self.post(url, post_data)
            return format(round(float(re.findall(r'\d+.\d+', resp.json()[0])[0]), 4), '.{0}f'.format(precision))
        except:
            return 0.0000
    
    def get_nfts(self, account: str):
        url = 'https://wax.api.atomicassets.io/atomicassets/v1/assets?collection_name=alien.worlds&owner={0}&page=1&limit=50&order=desc&sort=asset_id'.format(account)
        resp = requests.get(url)
        ids = []
        for nft in resp.json()['data']:
            ids.append(nft['asset_id'])
    ...
    def make_transaction(self, trx: Dict) -> Transaction:
        ...
        if self.cpu_payer == 0:
            ...
            actors.insert(0, "{0}-{1}".format(self.cpu_payer.account, self.cpu_payer.permission))
